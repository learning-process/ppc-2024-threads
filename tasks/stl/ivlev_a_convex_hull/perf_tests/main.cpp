...
TEST(ivlev_a_convex_hull_perf_test_stl, test_pipeline_run_stl) {
  // Create data
  ...
  // Create Task
  auto testTaskParallel = std::make_shared<ConvexHullSTLTaskParallel>(taskDataPar);
...
TEST(ivlev_a_convex_hull_perf_test_stl, test_task_run_stl) {
  // Create data
  std::vector<std::vector<std::pair<size_t, size_t>>> in = {{}};
  in[0].emplace_back(0, 1);
  in[0].emplace_back(1, 0);
