...
bool ConvexHullSTLTaskSequential::pre_processing() {
  internal_order_test();
  try {
...
bool ConvexHullSTLTaskSequential::validation() {
  internal_order_test();
  try {
...
bool ConvexHullSTLTaskSequential::run() {
  internal_order_test();
  try {
    for (size_t i = 0; i < taskData->inputs_count[0]; i++) {
...
bool ConvexHullSTLTaskSequential::post_processing() {
  internal_order_test();
  try {
    size_t n = taskData->inputs_count[0];
...
std::vector<std::pair<size_t, size_t>> ConvexHullSTLTaskSequential::Convex_Hull(
