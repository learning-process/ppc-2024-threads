...
TEST(shipitsin_l_rect_integr_seq, Small) {
  ...
  // Create Task
  ShipitsinTaskSequential shipitsinTaskSequential(taskDataSeq);
  ASSERT_EQ(shipitsinTaskSequential.validation(), true);
  shipitsinTaskSequential.pre_processing();
  shipitsinTaskSequential.run();
  shipitsinTaskSequential.post_processing();
  ASSERT_NEAR((double)5 / 6, out[0], out[1]);
}

TEST(shipitsin_l_rect_integr_seq, Normal) {
...
TEST(shipitsin_l_rect_integr_seq, Zero) {
...
TEST(shipitsin_l_rect_integr_seq, Negative) {
...
TEST(shipitsin_l_rect_integr_seq, NegativeLarge) {
