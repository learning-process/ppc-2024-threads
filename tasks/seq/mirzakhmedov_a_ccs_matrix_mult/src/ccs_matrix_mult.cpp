...
bool CSeq::run() {
  ...
  for (int b_col = 0; b_col < C->numCols; ++b_col) {
    ...
    int col_nonZeros_count = 0;
    for (int c_row = 0; c_row < C->numRows; ++c_row) {
      col_nonZeros_count += P_elem[c_row];
    }
    C->columnPointers[b_col + 1] = col_nonZeros_count + C->columnPointers[b_col];
  }
  int total_nonZeros = C->columnPointers[C->numCols];
  C->numNonZeros = total_nonZeros;
  C->rowIndices.resize(total_nonZeros);
  C->nonzeroValues.resize(total_nonZeros);
