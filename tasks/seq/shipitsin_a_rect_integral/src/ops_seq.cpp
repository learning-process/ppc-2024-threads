...
double shipitsin_flin(double x, double y) { return x - y; }
double shipitsin_fxy(double x, double y) { return x * y; }
double shipitsin_fysinx(double x, double y) { return y * std::sin(x); }
double shipitsin_fxexpy(double x, double y) { return x * std::exp(y); }

bool ShipitsinTaskSequential::pre_processing() {
  internal_order_test();
  try {
...
bool ShipitsinTaskSequential::validation() {
  internal_order_test();
  return taskData->inputs_count[0] == 4 && taskData->outputs_count[0] == 1;
}

bool ShipitsinTaskSequential::run() {
  internal_order_test();
  try {
...
bool ShipitsinTaskSequential::post_processing() {
  internal_order_test();
  try {
