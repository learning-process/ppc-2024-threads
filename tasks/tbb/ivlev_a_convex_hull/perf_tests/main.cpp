...
TEST(ivlev_a_convex_hull_perf_test_tbb, test_pipeline_run_tbb) {
  // Create data
  ...
  // Create Task
  auto testTaskParallel = std::make_shared<ConvexHullTBBTaskParallel>(taskDataPar);
...
TEST(ivlev_a_convex_hull_perf_test_tbb, test_task_run_tbb) {
  // Create data
  std::vector<std::vector<std::pair<size_t, size_t>>> in = {{}};
  in[0].emplace_back(0, 1);
  in[0].emplace_back(1, 0);
